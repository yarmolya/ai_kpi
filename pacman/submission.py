...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def performExpectimax(self, depth: int, agentIndex: int, gameState: GameState) -> float:
        ...
        for action in todo:
            ...
            if agentIndex + 1 >= gameState.getNumAgents():
                ret.append(self.performExpectimax(depth + 1, 0, successor))
            else:
                ret.append(self.performExpectimax(depth, agentIndex + 1, successor))

        if agentIndex == 0:
            if depth == 1:
                maxscore = max(ret)
                return todo[ret.index(maxscore)]
            else:
                return max(ret)
...
def betterEvaluationFunction(currentGameState: GameState) -> float:
    ...
    # Add a penalty for each ghost based on the sum of distances to all ghosts
    totalDistanceToGhosts = sum(manhattanDistance(pacmanPos, ghostState.getPosition()) for ghostState in ghostStates)
    score -= totalDistanceToGhosts
